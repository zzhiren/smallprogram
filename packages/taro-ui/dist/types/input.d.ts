...
export interface InputProps extends Component, OmitInputProps {
  /**
  ...
  label?: string
  /**
   * 是否自定义label true|false
   * @default false
   */
  customLabel?: boolean
  /**
   * 标题显示在左侧还是顶部，left|top
   * @default 'left'
   */
  align?: string
  /**
   ...
   * @default 'text'
   */
  type?: 'text' | 'number' | 'password' | 'phone' | 'idcard' | 'digit' | undefined
  /**
   * 错误提示信息，如果与renderError一起使用则以renderError优先展示
   * @default ''
   */
  error?: string
  /**
   ...
   */
  onClick?: InputFunction<string | number, InputEventDetail, any>
  /**
   * 键盘高度发生变化的时候触发此事件
   */
  onKeyboardHeightChange?: (
